...
<div class="contents-wrap">
	...
	<%} else {%>
			<input type="button" value="이전" onclick="location.href='RequestList.request?page=<%=nowPage - 1 %>'" class = "on fir">
	<%} %>
	
	<%for(int i = startPage; i <= endPage; i++) { 
			if(i == nowPage) { %>
				<a href="RequestList.request?page=<%=i %>" class = "on fir" data-page-num="1"><%=i %>&nbsp;</a>
			<%} else { %>
					<a href="RequestList.request?page=<%=i %>" class = "on fir">[<%=i %>]</a>&nbsp;
			<%} %>
	<%} %>
	...
	<%} else { %>
			<input type="button" value="다음" onclick="location.href='RequestList.request?page=<%=nowPage + 1 %>'" class = "on fir">
	<%} %>
	</section>
	</div>
	<%
